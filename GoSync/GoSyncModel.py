...
class GoSyncModel(object):
    def __init__(self):
        ...
        self.SendToLog(3,"Initialize - Started Initialize")
        ...
        #check if Credentials.json file exists
            self.SendToLog(3,"Initialize - Missing Credentials File")
            if (self.AskChooseCredentialsFile()):
                self.SendToLog(3,"Initialize - Ask Location of Credentials File")
                if (self.getCredentialFile() == False) :
                    self.SendToLog(1,"Initialize - Failled to load Credentials File")
                    raise ClientSecretsNotFound()
            else:
                self.SendToLog(3,"Initialize - Declined to Locate Credentials File")
                raise ClientSecretsNotFound()
        self.SendToLog(2,"Initialize - Completed Credentials Verification")

        self.SendToLog(2, "Initialize - Saving credentials")
        ...
        self.SendToLog(2, "Initialize - starting oberserver")
        self.observer = Observer()
        self.SendToLog(2, "Initialize - Going for authentication")
        self.DoAuthenticate()
        self.about_drive = self.drive.about().get(fields='user, storageQuota').execute()
        self.SendToLog(2,"Initialize - Completed Drive Quota Execution")

        self.user_email = self.about_drive['user']['emailAddress']
        self.SendToLog(3,"Initialize - Completed Account Information Load")

        self.tree_pickle_file = os.path.join(self.config_path, 'gtree-' + self.user_email + '.pick')

        self.SendToLog(3, "Initialize - Loaded tree pickle")

        if not os.path.exists(self.config_file):
            self.SendToLog(3, "Initialize - Creating default config file")
            self.CreateDefaultConfigFile()
            self.SendToLog(3,"Initialize - Completed Default Config File Creation")
#todo : add default config logic in method
        try:
            self.SendToLog(3, "Initialize - Trying to load config file %s" % self.config_file)
            self.LoadConfig()
            self.SendToLog(3,"Initialize - Read %s as base mirror" % self.base_mirror_directory)
            ...
            self.SendToLog(3,"Initialize - Completed mirror_directory validation")
            self.SendToLog(3,"Initialize - Mirror Directory: %s" % self.mirror_directory)
            if not self.sync_selection:
                self.can_autostart = False
        except:
            self.SendToLog(2, "Initialize - Exception during configuration load")
            raise

        self.SendToLog(3,"Initialize - Completed Config File Load")
...
# Depends on Log_Level constant
    def SendToLog(self, LogType, LogMsg):
    ...
    def SetTheBallRolling(self):
        ...
        if self.can_autostart and self.auto_start_sync:
            self.SendToLog(2, "SetTheBallRolling: Starting sync")
            self.StartSync()
        else:
            self.StopSync()
                ...
                try:
                    ...
                    if self.config_dict['SyncInterval']:
                        ...
                        if self.sync_interval < 30 or self.sync_interval > (24*60*60):
                            self.SendToLog(3, "LoadConfig: Setting sync to default value")
                            self.sync_interval = 1800

                    self.SendToLog(3, "Sync Interval: %d seconds" % self.sync_interval)

                    if self.config_dict['LogLevel']:
                        lvl = self.config_dict['LogLevel']
                        if lvl > 3:
                            self.SendToLog(2, "LoadConfig: Setting log level to default (%s)" % Default_Log_Level)
                            self.Log_Level = Default_Log_Level
                        else:
                            self.SendToLog(2, "LoadConfig: Log level is %d" % lvl)
                            self.Log_Level = lvl
                    else:
                        self.SendToLog(2, "LoadConfig: Not log level set. Setting to default")
                        self.Log_Level = Default_Log_Level
                    ...
                    if self.config_dict['BaseMirrorDirectory']:
                        ...
                        self.SendToLog(3, "Initialize - Base Mirror: %s" % self.base_mirror_directory)
                    else:
                        self.SendToLog(3, "LoadConfig: BaseMirrorDirectory not set")

                    self.sync_selection = self.config_dict['Sync Selection']
                    if not self.config_dict['AutoStartSync']:
                        self.SendToLog(2, "LoadConfig: Autostart of sync is disabled")
                        self.auto_start_sync = False
                    else:
                        self.auto_start_sync = self.config_dict['AutoStartSync']
                        if self.auto_start_sync:
                            self.SendToLog(2, "LoadConfig: Autostart is enabled")
                        else:
                            self.SendToLog(2, "LoadConfig: Autostart is disabled")
                    try:
        ...
        try:
            ...
            if os.path.exists(self.client_pickle):
                with open(self.client_pickle, 'rb') as token:
                    self.SendToLog(2, "Authenticate - Loading pickle file")
                    try:
                        creds = pickle.load(token)
                        self.SendToLog(2, "Authenticate - Loading pickle file: SUCCESS")
                    except:
                        self.SendToLog(2, "Authenticate - Failed to load pickle file")
                        creds = None

            # If there are no (valid) credentials available, let the user log in.
            if not creds or not creds.valid:
                if creds and creds.expired and creds.refresh_token:
                    self.SendToLog(2, "Authenticate - expired. Refreshing")
                    creds.refresh(Request())
                else:
                    self.SendToLog(2, "Authenticate - New authentication")
                    self.SendToLog(2, "Authenticate - File %s" % (self.credential_file))
                    flow = InstalledAppFlow.from_client_secrets_file(self.credential_file, SCOPES)
                    self.SendToLog(2, "Authenticate - running local server")
                    creds = flow.run_local_server(port=0)
                self.SendToLog(2, "Authenticate - Saving pickle file")
                # Save the credentials for the next run
                with open(self.client_pickle, 'wb') as token:
                    pickle.dump(creds, token)

            self.SendToLog(2, "Authenticate - Building service")
            try:
                service = build('drive', 'v3', credentials=creds)
                self.SendToLog(2, "Authenticate - service built successfully!")
            except:
                self.SendToLog(2, "Authenticate - service built failed. Going for re-authentication")
                try:
                    flow = InstalledAppFlow.from_client_secrets_file(self.credential_file, SCOPES)
                    creds = flow.run_local_server(port=0)
                    service = build('drive', 'v3', credentials=creds)
    ...
    def CreateDirectoryByPath(self, dirpath):
        self.SendToLog(3,"create directory: %s\n" % dirpath)
    ...
    def CreateRegularFile(self, file_path, parent='root', uploaded=False):
        self.SendToLog(3,"Create file %s\n" % file_path)
    ...
    def UploadFolder(self, a_dirpath, addToTree=True):
        self.SendToLog(3, "UploadFolder: %s" % a_dirpath)
        dirpath=self.GetRelativeFolder(a_dirpath, True)
        self.SendToLog(3, "UploadFolder: %s (Relative: %s)" % (a_dirpath, dirpath))

        self.UploadFile(a_dirpath)
        while True:
            try:
                self.SendToLog(3, "UploadFolder: Locating new directory %s in remote"
                                % dirpath)
                nf = self.LocateFolderOnDrive(dirpath)
                self.sync_selection.append([dirpath, nf['id']])
                self.SendToLog(3, "UploadFolder: New Child: %s ID: %s" % (nf['name'], nf['id']))
                self.SendToLog(2, "UploadFolder: Added newly uploaded directory %s in sync list" % dirpath)
                parent = os.path.dirname(dirpath)
                if parent == '':
                    parent = 'root'
                    self.SendToLog(3, "UploadFolder: Find new child's parent: %s, child: %s (%s)"
                                    % (parent, nf['name'], nf['id']))
                    self.driveTree.AddFolder(parent, nf['id'], nf['name'], nf)
                    self.SendToLog(2, "UploadFolder: Added to tree")
                    GoSyncEventController().PostEvent(GOSYNC_EVENT_CALCULATE_USAGE_DONE, 0)
                else:
                    parent_f = self.LocateFolderOnDrive(parent)
                    self.SendToLog(3, "UploadFolder: Find new child's parent: %s (%s)"
                                    % (parent_f['name'], parent))
                    self.driveTree.AddFolder(parent_f['id'], nf['id'], nf['name'], nf)
                    self.SendToLog(3, "UploadFolder: Added new child %s to parent %s"
                                    % (nf['name'], parent_f['name']))
                    GoSyncEventController().PostEvent(GOSYNC_EVENT_CALCULATE_USAGE_DONE, 0)
                break
            except InternetNotReachable:
                self.SendToLog(1, "UploadFolder: Internet down")
                while True:
                    time.sleep(5)
                ...
                self.SendToLog(1, "UploadFolder: Unknown exception: Trying to get ID of new directory")
                raise

    def UploadFile(self, file_path):
        self.SendToLog(3, "UploadFile: %s" % file_path)
        if os.path.isfile(file_path):
            drivepath = file_path.split(self.mirror_directory+'/')[1]
            self.SendToLog(3,"file: %s drivepath is %s\n" % (file_path, drivepath))
            try:
                f = self.LocateFileOnDrive(drivepath)
                self.SendToLog(3,'Found file %s on remote (dpath: %s)\n' % (f['name'], drivepath))
                newfile = False
                self.SendToLog(3,'Checking if they are same... ')
                if f['md5Checksum'] == self.HashOfFile(file_path):
                    self.SendToLog(3,'yes\n')
                    return
                else:
                    self.SendToLog(3,'no\n')
            except (FileNotFound, FolderNotFound):
                self.SendToLog(3,"A new file!\n")
                newfile = True
            except:
                raise
            ...
            if dirpath == '':
                self.SendToLog(3,'Creating %s file in root\n' % file_path)
                try:
                    ...
                    self.SendToLog(1, "CreateRegularFile: Failed to upload %s" % file_path)
                ...
                try:
                    ...
                    self.SendToLog(1, "CreateRegularFile: Failed to upload %s in %s" % file_path, dir_path)
                    raise RegularFileUploadFailed()
        else:
    ...
    def UploadObservedFile(self, file_path):
        if self.IsSyncRunning():
            self.SendToLog(3, "UploadObservedFile: File %s is created but sync is running. Possibly created by sync. Skipping."
                            % file_path)
            return
        ...
        try:
            ...
            self.SendToLog(1, "UploadObservedFile - Internet is down")
        except:
            self.SendToLog(1, "UploadObservedFile - Failed to upload file %s\n" % file_path)

        self.SendToLog(3, "UploadObservedFile - Releasing lock")
    ...
    def RenameFile(self, file_object, new_title):
        try:
            ...
            self.SendToLog(1,'An error occurred while renaming file: %s' % error)
            return None
        except:
    ...
    def RenameObservedFile(self, file_path, new_name):
        ...
        self.SendToLog(3,"RenameObservedFile: Rename %s to new name %s\n"
                          % (file_path, new_name))
        try:
            ftd = self.LocateFileOnDrive(drive_path)
            nftd = self.RenameFile(ftd, new_name)
            if not nftd:
                self.SendToLog(1,"File rename failed\n")
        except:
    ...
    def TrashFile(self, file_object):
        try:
            ...
            self.SendToLog(3,{"TRASH_FILE: File %s deleted successfully.\n" % file_object['name']})
        except errors.HttpError as error:
            self.SendToLog(1,"TRASH_FILE: HTTP Error\n")
            raise RegularFileTrashFailed()

    def TrashFileCallback(self, Folder):
        self.SendToLog(3, "TrashFileCallback: Folder: %s being deleted" % Folder.GetPath())
        if Folder and Folder.GetPath() in self.sync_selection:
            self.sync_selection.remove(Folder.GetPath())
            self.SendToLog(3, "TrashFileCallback: Folder %s deleted from sync list" % Folder.GetPath())

    def TrashObservedFile(self, file_path):
        if self.IsSyncRunning():
            self.SendToLog(3, "TrashObservedFile: File %s Deleted but sync is running" % file_path)
            return

        self.sync_lock.acquire()
        drive_path = file_path.split(self.mirror_directory+'/')[1]
        self.SendToLog(3,{"TRASH_FILE: dirpath to delete: %s\n" % drive_path})
        try:
            ftd = self.LocateFileOnDrive(drive_path)
            if not ftd:
                self.SendToLog(1,{"TRASH_FILE: invalid file handle for %s\n" % drive_path})

            try:
                if ftd['mimeType'] == 'application/vnd.google-apps.folder':
                    self.SendToLog(3, "Deleting folder %s (%s) from local drive tree"
                ...
                self.SendToLog(1,{"TRASH_FILE: Failed to move file %s to trash\n" % drive_path})
                raise
            except:
                raise
        except (FileNotFound, FileListQueryFailed, FolderNotFound):
            self.SendToLog(1,{"TRASH_FILE: Failed to locate %s file on drive\n" % drive_path})
            pass
        except:
            self.SendToLog(1,{"TRASH_FILE: Unknown exception for file %s (%s)\n" % (drive_path, file_path)})
    ...
    def MoveObservedFile(self, src_path, dest_path):
        ...
        self.SendToLog(3,"Moving file %s to %s\n" % (from_drive_path, to_drive_path))

        try:
            ftm = self.LocateFileOnDrive(from_drive_path)
            self.SendToLog(3,"MoveObservedFile: Found source file on drive\n")
            if os.path.dirname(from_drive_path) == '':
                sf = 'root'
            else:
                sf = self.LocateFolderOnDrive(os.path.dirname(from_drive_path))
            self.SendToLog(3,"MoveObservedFile: Found source folder on drive\n")
            try:
                if to_drive_path == '':
                    df = 'root'
                else:
                    df = self.LocateFolderOnDrive(to_drive_path)
                self.SendToLog(3,"MoveObservedFile: Found destination folder on drive\n")
                try:
                    self.SendToLog(3,"MovingFile() ")
                    self.MoveFile(ftm, df, sf)
                    self.SendToLog(3,"done\n")
                except (Unkownerror, FileMoveFailed):
                    self.SendToLog(1,"MovedObservedFile: Failed\n")
                    return
                except:
                    self.SendToLog(1,"?????\n")
                    return
            except FolderNotFound:
                self.SendToLog(1,"MoveObservedFile: Couldn't locate destination folder on drive.\n")
                return
            except:
                self.SendToLog(1,"MoveObservedFile: Unknown error while locating destination folder on drive.\n")
                return
        except FileNotFound:
                self.SendToLog(1,"MoveObservedFile: Couldn't locate file on drive.\n")
                return
        except FileListQueryFailed:
            self.SendToLog(1,"MoveObservedFile: File Query failed. aborting.\n")
            return
        except FolderNotFound:
            self.SendToLog(1,"MoveObservedFile: Folder not found\n")
            return
        except:
            self.SendToLog(1,"MoveObservedFile: Unknown error while moving file.\n")
            return
...
#### LocateFileInFolder
    def LocateFileInFolder(self, filename, parent='root'):
        try:
            self.SendToLog(3, "LocateFileInFolder - Querying remote\n")
            file_list = self.MakeFileListQuery("'%s' in parents and trashed=false" % parent)
            for f in file_list:
                if f['name'] == filename:
                    self.SendToLog(3, "LocateFileInFolder - Found\n")
                    return f
            self.SendToLog(1, "LocateFileInFolder - %s not found\n" % filename)
            raise FileNotFound()
        except InternetNotReachable:
            self.SendToLog(1, "Internet is down\n")
            raise
        except:
            self.SendToLog(3, "Raising FileNotFound\n")
            raise FileNotFound()
...
#### LocateFileOnDrive
    def LocateFileOnDrive(self, abs_filepath):
        ...
        if dirpath != '':
            try:
                self.SendToLog(3, "LocateFileOnDrive - locating %s directory on remote\n" % dirpath)
                f = self.LocateFolderOnDrive(dirpath)
                try:
                    self.SendToLog(3, "LocateFileOnDrive - locating %s file\n" % f['id'])
                    fil = self.LocateFileInFolder(filename, f['id'])
                    self.SendToLog(3, "LocateFileOnDrive - File found\n")
                    return fil
                except InternetNotReachable:
                    self.SendToLog(3,"LocateFileOnDrive - Internet seems to be down!\n")
                    raise
                except FileNotFound:
                    self.SendToLog(3,"LocateFileOnDrive - Local File (%s) not in remote." % filename)
                    raise
                except FileListQueryFailed:
                    self.SendToLog(3,"LocateFileOnDrive - Locate File (%s) list query failed" % filename)
                    raise
            except InternetNotReachable:
                self.SendToLog(3, "LocateFileOnDrive (Folder) - Internet seems to be down!\n")
                raise
            except FolderNotFound:
                self.SendToLog(3,"LocateFileOnDrive - Local Folder (%s) not in remote" % dirpath)
                raise
            except FileListQueryFailed:
                self.SendToLog(3,"LocateFileOnDrive - Locate Folder (%s) list query failed" % dirpath)
                raise
        else:
            try:
                #self.SendToLog(3, "LocateFileOnDrive - locating %s file\n" % f['id'])
                ...
                self.SendToLog(3, "LocateFileOnDrive (File) - Internet seems to be down!\n")
                raise
            except FileNotFound:
                self.SendToLog(3,"LocateFileOnDrive: Local File (%s) not in remote." % filename)
                raise
            except FileListQueryFailed:
                self.SendToLog(3,"LocateFileOnDrive: File (%s) list query failed." % filename)
                raise
            except:
                self.SendToLog(1,"LocateFileOnDrive: Unknown error in locating file (%s) in local folder (root)" % filename)
                raise
...
#### GetFolderOnDrive
    def GetFolderOnDrive(self, folder_name, parent='root'):
        ...
        mentioned in parent.
        """
        self.SendToLog(3,"GetFolderOnDrive: Checking Folder (%s) on (%s)" % (folder_name, parent))
        try:
            file_list = self.MakeFileListQuery("'%s' in parents and trashed=false"  % parent)
            for f in file_list:
                if f['name'] == folder_name and f['mimeType']=='application/vnd.google-apps.folder':
                    self.SendToLog(3,"GetFolderOnDrive: Found Folder (%s) on (%s)" % (folder_name, parent))
                    return f
        except InternetNotReachable:
            raise
        except:
...
#### SyncLocalDirectory
    def SyncLocalDirectory(self):
        if not self.syncRunning.is_set() or self.shutting_down:
            self.SendToLog(3,"SyncLocalDirectory: Sync has been paused. Aborting.\n")
            return

        self.SendToLog(3,"### SyncLocalDirectory: - Sync Started")
        for root, dirs, files in os.walk(self.mirror_directory):
            if not self.IsDirectoryMonitored(root):
                self.SendToLog(2, "SyncLocalDirectory - Directory %s is not monitored. Deleting Locally" % root)
            ...
            for names in files:
                while True:
                    ...
                    try:
                        ...
                        drivepath = dirpath.split(self.mirror_directory+'/')[1]
                        self.SendToLog(3,"SyncLocalDirectory: Checking Local File (%s)" % drivepath)
                        f = self.LocateFileOnDrive(drivepath)
                        self.SendToLog(2,"SyncLocalDirectory: Skipping Local File (%s) same as Remote\n" % dirpath)
                        break
                    except FileListQueryFailed:
                        ...
                        self.SendToLog(2,"SyncLocalDirectory: Remote File (%s) Check Failed. Aborting.\n" % dirpath)
                        return
                    except InternetNotReachable:
                        self.SendToLog(2, "SyncLocalDirectory: Network is down!\n")
                        ...
                        while True:
                            if self.IsInternetReachable():
                                ...
                                self.SendToLog(2, "SyncLocalDirectory: Network is up!\n")
                                break
                            else:
                                time.sleep(5)
                                continue
            ...
            for names in dirs:
                ...
                self.SendToLog(3, "Checking Local Folder: %s" % names)
                ...
                try:
                    ...
                    # its gone in remote drive. Let the next sync come and take care of this
                    # Log the event though
                    self.SendToLog(2,"SyncLocalDirectory: Remote Folder (%s) Check Failed. Aborting.\n" % dirpath)
                    return
                except InternetNotReachable:
                    self.SendToLog(1, "SyncLocalDirectory: Internet seems to be down!\n")
                    raise
                except:
                    if os.path.exists(dirpath) and os.path.isdir(dirpath):
                        self.SendToLog(3,"SyncLocalDirectory: Uploading Local Folder (%s) - Not in Remote\n" % dirpath)
                        try:
                            GoSyncEventController().PostEvent(GOSYNC_EVENT_SYNC_UPDATE, {"Creating Folder: %s" % f['name']})
                            self.UploadFolder(dirpath)
                        except:
                            self.SendToLog(1, "SyncLocalDirectory: Failed to upload local folder (%s)\n" % dirpath)
                            raise

        self.SendToLog(3,"### SyncLocalDirectory: - Sync Completed")
    ...
    def MakeFileListQuery(self, query):
        ...
        while True:
            try:
                ...
                self.SendToLog(1, "MakeFileListQuery - %s\n", error.resp.reason)
                if error.resp.status in [403, 500, 503, 429]:
                    self.SendToLog(1, "MakeFileListQuery - Status: %d. (Retrying)\n", error.resp.status)
                    time.sleep(5)
                    continue

                if not self.IsInternetReachable():
                    self.SendToLog(1, "MakeFileListQuery - Internet is down\n")
                    raise InternetNotReachable()
            except:
                if not self.IsInternetReachable():
                    self.SendToLog(1, "MakeFileListQuery (unknown except) - Internet is down\n")
                    raise InternetNotReachable()
                else:
                    if retry == 0:
                        self.SendToLog(1, "MakeFileListQuery - Query failed. Trying one more time.")
                        retry = 1;
                        continue
                    else:
                        self.SendToLog(1, "MakeFileListQuery (unknown except %d) - Raising FileListQueryFailed", e.errno)
    ...
    def IsDirectoryMonitored(self, dir):
        if self.IsMonitoringAll():
            self.SendToLog(3, "IsDirectoryMonitored: Complete monitoring is on! (%s)" % dir)
            return True
        ...
        try:
            ...
            for d in self.sync_selection:
                if d[0] == dirpath:
                    self.SendToLog(3, "IsDirectoryMonitored: Match: SL: %s dirpath: %s" % (d[0], dirpath))
                    return True
                else:
                    continue

            self.SendToLog(3, "IsDirectoryMonitored: No match: dirpath: %s" % (dirpath))
            return False
        except:
            self.SendToLog(3, "IsDirectoryMonitored: %s is in root. Always monitored!" % dir)
            # This is root directory. Which is always monitored
            return True

    def IsFilePathMonitored(self, path):
        if self.IsMonitoringAll():
            self.SendToLog(3, "IsFilePathMonitored: Path: %s. Complete mirror directory is monitored" % path)
            return True
        ...
        if dirpath == '':
            self.SendToLog(3, "IsFilePathMonitored: Path: %s is in ROOT. Always monitored" % path)
            return True
        else:
            for e in self.sync_selection:
                if e[0] == dirpath:
                    self.SendToLog(3, "IsFilePathMonitored: Path: %s is monitored" % dirpath)
                    return True
                else:
                    continue

            self.SendToLog(3, "IsFilePathMonitored: Path: %s is not monitored" % dirpath)
            return False
...
#### DownloadFileByObject
    ...
    def DownloadFileByObject(self, file_obj, download_path):
        ...
        def RetryAndContinue(retryCounter, exceptionMsg):
            retryCounter -= 1
            if retryCounter > 0 :
                self.SendToLog(1, "DownloadFileByObject: Download error (%s). Retrying... (%s)" % (exceptionMsg,str(retryCounter)))
                time.sleep(5)
            else:
                self.SendToLog(1, "DownloadFileByObject: Download error (%s). Aborting..." % exceptionMsg)
            return retryCounter
...
#### SyncRemoteDirectory
    def SyncRemoteDirectory(self, parent, pwd, recursive=True):
        self.SendToLog(3,"### SyncRemoteDirectory: - Sync Started - Remote Directory (%s) ... Recursive = %s\n" % (pwd, recursive))
        if not self.syncRunning.is_set() or self.shutting_down:
            self.SendToLog(2,"SyncRemoteDirectory: Sync has been paused. Aborting.\n")
            return
        ...
        try:
            ...
            for f in file_list:
                if not self.syncRunning.is_set() or self.shutting_down:
                    self.SendToLog(3,"SyncRemoteDirectory: Sync has been paused. Aborting.\n")
                    return

                self.SendToLog(3, "Checking: %s\n" % f['name'])
                GoSyncEventController().PostEvent(GOSYNC_EVENT_SYNC_UPDATE, {"Checking: %s" % f['name']})
                ...
                if f['mimeType'] == 'application/vnd.google-apps.folder':
                    ...
                    self.SendToLog(3,"SyncRemoteDirectory: Checking directory (%s)" % f['name'])
                    if not os.path.exists(abs_dirpath):
                        self.SendToLog(3,"SyncRemoteDirectory: Creating directory (%s)" % abs_dirpath)
                        os.makedirs(abs_dirpath)
                        self.SendToLog(3,"SyncRemoteDirectory: Created directory (%s)" % abs_dirpath)
                    self.SendToLog(3,"SyncRemoteDirectory: Syncing directory (%s)\n" % f['name'])
                    while True:
                        try:
                            ...
                            GoSyncEventController().PostEvent(GOSYNC_EVENT_INTERNET_UNREACHABLE, 1)
                            self.SendToLog(1, "SyncRemoteDirectory - Network has gone down")
                            while True:
                                if self.IsInternetReachable():
                                    GoSyncEventController().PostEvent(GOSYNC_EVENT_INTERNET_UNREACHABLE, 0)
                                    self.SendToLog(2, "SyncRemoteDirectory - Network is up!")
                                    break
                                else:
                                    time.sleep(5)
                                    continue
                    ...
                    self.SendToLog(3,"SyncRemoteDirectory: Checking file (%s)" % f['name'])
                    if not self.IsGoogleDocument(f):
                        self.DownloadFileByObject(f, os.path.join(self.mirror_directory, pwd))
                    else:
                        self.SendToLog(3,"SyncRemoteDirectory: Skipping file (%s) is a google document.\n" % f['name'])
        except InternetNotReachable:
            self.SendToLog(1, "SyncRemoteDirectory: Internet not reachable\n")
            raise
        except:
...
#### run (Sync Local and Remote Directory)
    def run(self):
        while not self.shutting_down:
            self.SendToLog(3, "SyncThread - run - Waiting for Sync to be enabled")
            ...
            if not self.IsInternetReachable():
                self.SendToLog(2, "SyncThread - run - Internet is down. Clearing running.")
                GoSyncEventController().PostEvent(GOSYNC_EVENT_INTERNET_UNREACHABLE, 1)
                while True:
                    if not self.IsInternetReachable():
                        time.sleep(5)
                    else:
                        self.SendToLog(2, "SyncThread - run - Internet is up!")
                        GoSyncEventController().PostEvent(GOSYNC_EVENT_INTERNET_UNREACHABLE, 0)
                        break

            self.SendToLog(3, "SyncThread - run - Trying to acquire lock.")
            self.sync_lock.acquire()
            self.SendToLog(3, "SyncThread - run - Lock acquired.")

            try:
                self.SendToLog(3, "SyncThread - run - Validating sync settings")
                self.validate_sync_settings()
                self.SendToLog(2, "SyncThread - run - validated")
            except InternetNotReachable:
                self.SendToLog(2, "SyncThread - run - Validate sync settings => Internet is down")
                self.sync_lock.release()
                continue
            except FolderNotFound as f:
                self.SendToLog(2, "SyncThread - run - Validate sync settings => Folder %s not found" % f.e_folder)
            ...
            try:
                ...
                if self.updates_done:
                    self.SendToLog(2,"Sync - Some changes were done. Triggering drive usage calculation.\n")
                    self.usageCalculateEvent.set()
                GoSyncEventController().PostEvent(GOSYNC_EVENT_SYNC_DONE, 0)
            except InternetNotReachable:
                self.SendToLog(2, "SyncThread - run - Internet not reachable")
                GoSyncEventController().PostEvent(GOSYNC_EVENT_INTERNET_UNREACHABLE, 1)
                self.sync_lock.release()
                self.syncing_now = False
                continue
            except:
                self.SendToLog(1, "SyncThread - run - Unknown exception")
                GoSyncEventController().PostEvent(GOSYNC_EVENT_SYNC_DONE, -1)

            self.SendToLog(2, "SyncThread - run - Sync done")
            self.sync_lock.release()
            self.syncing_now = False
...
#Migration V3 API
#            self.SendToLog(1,"Failed to get size of file %s (mime: %s)\n" % (f['title'], f['mimeType']))
            self.SendToLog(1,"Failed to get size of file %s (mime: %s)\n" % (f['name'], f['mimeType']))
            return 0
...
#### calculateUsage
    def calculateUsage(self):
        while not self.shutting_down:
            ...
            if self.shutting_down:
                self.SendToLog(2, "calculateUsage - GoSync is shutting down!")
                break

            self.sync_lock.acquire()
            self.SendToLog(3,"CalculateUsage: SyncLock acquired")
            ...
            if self.drive_usage_dict and not self.updates_done:
                self.SendToLog(3,"CalculateUsage: No calculation to be done")
                GoSyncEventController().PostEvent(GOSYNC_EVENT_CALCULATE_USAGE_DONE, 0)
                self.sync_lock.release()
                continue

            self.SendToLog(3,"CalculateUsage: Started")
            self.updates_done = 0
            ...
            try:
                ...
                GoSyncEventController().PostEvent(GOSYNC_EVENT_CALCULATE_USAGE_DONE, -1)
                self.SendToLog(1,"Failed to get the total number of files in drive\n")
    ...
    def ForceDriveUsageCalculation(self):
        ...
        self.usageCalculateEvent.set()
        self.SendToLog(3,"ForceDriveUsageCalculation: Marked")
...
class FileModificationNotifyHandler(PatternMatchingEventHandler):
    ...
    def on_created(self, evt):
        self.sync_handler.SendToLog(3, "Observer: %s created\n" % evt.src_path)
        self.sync_handler.UploadObservedFile(evt.src_path)

    def on_moved(self, evt):
        self.sync_handler.SendToLog(2,"Observer: file %s moved to %s: Not supported yet!\n" % (evt.src_path, evt.dest_path))
        self.sync_handler.HandleMovedFile(evt.src_path, evt.dest_path)

    def on_deleted(self, evt):
        self.sync_handler.SendToLog(2,"Observer: file %s deleted on drive.\n" % evt.src_path)
